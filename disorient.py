...
def exif_orientation(f, set=None):
    ...
    buf = f.read(length - 8)
    #print `buf[:2]`
    if buf[:2] == 'II': is_motorola, order = 0, "<"
    elif buf[:2] == 'MM': is_motorola, order = 1, ">"
    else: return
    ...
    if is_motorola:
        if buf[2:4] != '\0\x2a': return
    else:
        if buf[2:4] != '\x2a\0': return
    ...
    while 1:
        ...
        if tagnum == 0x112: break
        number_of_tags -= 1
        if not number_of_tags: return 0
        offset += 12
...
def clear_exif_orientation(x): return exif_orientation(x, 1)

descr = {
    1: "Normal", 2: "Mirrored left-to-right", 3: "Rotated 180 degrees",
    4: "Mirrored top-to-bottom", 5: "Mirrored along top-left diagonal",
    6: "Rotated 90 degrees", 7: "Mirrored along top-right diagonal",
